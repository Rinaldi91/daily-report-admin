// app/medical-devices/[id]/page.tsx
...
interface GroupedHistoryRecord extends DeviceHistoryRecord {
  employees: string[];
}
...
export default function MedicalDeviceDetailPage() {
  ...
  // Tambahkan helper untuk mengelompokkan record berdasarkan report_number
  const groupedHistory: GroupedHistoryRecord[] = Object.values(
    filteredHistory.reduce((acc: Record<string, GroupedHistoryRecord>, rec) => {
      const reportNum = rec.report.report_number;

      if (!acc[reportNum]) {
        acc[reportNum] = {
          ...rec,
          employees: [rec.report.employee?.name].filter(
            (name): name is string => Boolean(name)
          ),
        };
      } else {
        if (
          rec.report.employee?.name &&
          !acc[reportNum].employees.includes(rec.report.employee.name)
        ) {
          acc[reportNum].employees.push(rec.report.employee.name);
        }
      }
      return acc;
    }, {})
  ...
  // Hitung total unik berdasarkan report_number dari semua history
  const uniqueTotalServiceCount = Object.keys(
    history.reduce((acc: Record<string, boolean>, rec) => {
      acc[rec.report.report_number] = true;
      return acc;
    }, {})
  ).length;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900">
        ...
        <p className="mt-4 text-gray-400">Loading Device Details...</p>
      </div>
    );
  }
    ...
    <div className="bg-gray-900 min-h-screen text-white p-4 sm:p-6 lg:p-8 rounded-2xl">
      <div className="max-w-9xl mx-auto">
        ...
        <div className="mb-10">
          {/* Back Button */}
          <button
            onClick={() => router.push("/dashboard/medical-devices")}
        ...
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400">
                ...
                {groupedHistory.length === 1 ? "record" : "records"} found found
              </p>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-white leading-tight">
                    {/* {groupedHistory.length} / {pagination.total_service_count} */}
                    {groupedHistory.length} / {uniqueTotalServiceCount}
                  </p>
          ...
          <div className="space-y-6">
            {groupedHistory.map((record) => (
              <div key={record.report.report_number} className="space-y-4">
