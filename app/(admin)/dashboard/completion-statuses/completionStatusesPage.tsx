...
export default function CompletionstatusPage() {
  ...
  const [formData, setFormData] = useState<FormCompletionStatus>({
    name: "",
    slug: "",
    description: "",
  });
  ...
  const fetchCompletionStatus = useCallback(
    async (page: number = 1, search: string = "") => {
      try {
        ...
        const completionStatusData = Array.isArray(json.data)
          ? json.data.map((p: Partial<CompletionStatus>) => ({
              id: p.id || 0,
              name: p.name || "",
              slug: p.slug || "",
        ...
        setCompletionStatuses(completionStatusData);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (res.ok) {
        ...
        Swal.fire({
          title: "Success",
          text: "Completiosn status has been saved",
          icon: "success",
          timer: 2000,
        ...
        if (errorData.errors) {
          ...
          Swal.fire(
            "Error",
            errorData.message || "Failed to save completion status",
            "error"
          );
        }
      }
  ...
  const handleDelete = async (slug: string) => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "You will not be able to recover this completion status!",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (res.ok) {
          ...
          Swal.fire(
            "Error",
            errorData.message || "Failed to delete completion status",
            "error"
          );
        }
      } catch (error) {
        console.error("Error deleting completion status:", error);
        Swal.fire("Error", "Failed to delete completion status", "error");
      }
    }
  };
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      const allIds = new Set(completionStatuses.map((item) => item.id));
  ...
  const handleMultiDelete = async () => {
    ...
    const selectedCompletionStatus = completionStatuses.filter((cat) =>
      selectedItems.has(cat.id)
    );
    const typeOfWorkNames = selectedCompletionStatus.map((cat) => cat.name).join(", ");

    const result = await Swal.fire({
      title: "Delete Multiple Completion Status?",
      html: `
            <p>You are about to delete <strong>${selectedItems.size}</strong> completion status:</p>
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        if (failedCount === 0) {
          Swal.fire({
            title: "Success!",
            text: `Successfully deleted ${successCount} completion status.`,
            icon: "success",
            timer: 3000,
            showConfirmButton: false,
            timerProgressBar: true,
          ...
          Swal.fire({
            ...
            text: `Deleted ${successCount} completion status successfully. ${failedCount} failed to delete.`,
            icon: "warning",
            background: "#1f2937",
            color: "#F9FAFB",
            customClass: {
        ...
        Swal.fire({
          title: "Error",
          text: "Failed to delete completion status. Please try again.",
          icon: "error",
          background: "#1f2937",
          color: "#F9FAFB",
        });
  ...
  useEffect(() => {
    ...
    searchTimeout.current = setTimeout(() => {
      fetchCompletionStatus(1, searchTerm);
    }, 400);
    return () => {
      if (searchTimeout.current) clearTimeout(searchTimeout.current);
    };
  }, [searchTerm, fetchCompletionStatus]);
  ...
  const handlePageChange = useCallback(
    (page: number) => {
      if (page >= 1 && page <= totalPages) {
        setCurrentPage(page);
        fetchCompletionStatus(page, searchTerm);
      }
    },
    [searchTerm, totalPages, fetchCompletionStatus]
  );
  ...
  const isAllSelected =
    completionStatuses.length > 0 && completionStatuses.every((cat) => selectedItems.has(cat.id));
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="text-white">
            <h1 className="text-2xl font-bold flex items-center gap-2">
              <ChartNetwork className="w-6 h-6" /> Completion Status Management
            </h1>
            <p className="mt-1 text-sm text-gray-400">
              Manage organizational completion status
            </p>
          </div>

          <div className="flex items-center gap-2">
            {selectedItems.size > 0 && hasPermission("delete-completion-status") && (
              <button
            ...
            {hasPermission("create-completion-status") && (
              <button
                onClick={handleAdd}
                className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors cursor-pointer"
              >
                <Plus className="w-4 h-4 mr-2" /> Add Completion Status
              </button>
            )}
          </div>
        </div>
        ...
        <div className="bg-gray-900 p-4 rounded-lg">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search completion status by name ..."
              value={searchTerm}
        ...
        <div className="bg-gray-900 rounded-lg overflow-hidden">
          <table className="w-full">
            <thead className="bg-gray-800 text-white">
              <tr>
                {hasPermission("delete-completion-status") && (
            ...
            <tbody className="divide-y divide-gray-800">
              {Array.isArray(completionStatuses) &&
                completionStatuses.map((completionStatus, index) => (
                  <tr key={completionStatus.id} className="hover:bg-gray-800">
                    {hasPermission("delete-completion-status") && (
                      <td className="px-6 py-4">
                        <input
                          type="checkbox"
                          checked={selectedItems.has(completionStatus.id)}
                          onChange={(e) =>
                            handleSelectItem(completionStatus.id, e.target.checked)
                          }
                          className="w-4 h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2 cursor-pointer"
                        />
                      </td>
                    ...
                    <td className="px-6 py-4 text-gray-400 text-sm">
                      {formatDate(completionStatus.created_at)}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {hasPermission("update-completion-status") && (
                          <button
                            onClick={() => handleEdit(completionStatus)}
                        ...
                        {hasPermission("delete-completion-status") && (
                          <button
                            onClick={() => handleDelete(completionStatus.slug)}
          ...
          {completionStatuses.length === 0 && !loading && (
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center min-h-[40vh] bg-gray-900 rounded-lg">
              ...
              <p className="mt-4 text-gray-400">Loading Completion status...</p>
            </div>
          )}
        </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Brief description of this completion status"
                  rows={3}
